...
print(paste("regions defined for", trait, "chr", chr))
print(region)
# step 2 merge overlapping regions
merged <- merge_region(region, window_mb = 1, data = dat.chr)
# plot the results
png(sprintf("/fs/projects/ukbb/yu/BOLT_basicQT_agesq/%s/sex_combined/chr%d/plots/merge_region_sex_combined_%s_chr%d.png",
            trait, chr, trait, chr))
...
write.table(merged, file = sprintf("/fs/projects/ukbb/yu/BOLT_basicQT_agesq/%s/sex_combined/chr%d/regions/merged_region_1mb.txt",
...
BOLT2z(top.snp, sprintf("/fs/projects/ukbb/yu/BOLT_basicQT_agesq/%s/sex_combined/chr%d/z_files/top_snp_sex_combined_%s_chr%d_wim_1mb",
                        trait, chr, trait, chr))
