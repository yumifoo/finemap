...
for(ii in seq_along(chromosomes)){
  chr <- chromosomes[ii]
  if(file.exists(sprintf("/fs/projects/ukbb/yu/BOLT_basicQT_agesq/%s/sex_combined/chr%d/lead_snp_cond/lead_snps_cond_chr%d.txt", trait, chr, chr))){ # check if file exists
